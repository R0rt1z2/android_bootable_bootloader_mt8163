...
void lcm_common_parse_dts(const LCM_DTS *DTS, unsigned char force_update)
{
	...
	if ((_LCM_DTS.parsing != 0) && (force_update == 0)) {
		dprintf(0, "[LCM][ERROR] %s: DTS has been parsed or non-force update: %d, %d \n", __func__, _LCM_DTS.parsing, force_update);
		return;
	}
...
#if defined(LCM_DEBUG)
	if (memcmp((unsigned char*)dts_params, (unsigned char*)(&(DTS->params)), sizeof(LCM_PARAMS)) != 0x0) {
		dprintf(0, "[LCM][ERROR] %s: DTS compare error \n", __func__);

		dprintf(0, "[LCM][ERROR] dts_params: \n");
		tmp = (unsigned char*)dts_params;
		for (i=0; i<sizeof(LCM_PARAMS); i+=8) {
			dprintf(0, "0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x \n", *(tmp+i), *(tmp+i+1), *(tmp+i+2), *(tmp+i+3), *(tmp+i+4), *(tmp+i+5), *(tmp+i+6), *(tmp+i+7));
		}

		dprintf(0, "[LCM][ERROR] DTS->params: \n");
...
//	dts_init = &(_LCM_DTS.init[0]);
	if (memcmp((unsigned char*)dts_init, (unsigned char*)(&(DTS->init[0])), sizeof(LCM_DATA)) != 0x0) {
		dprintf(0, "[LCM][ERROR] %s: DTS compare error \n", __func__);

		dprintf(0, "[LCM][ERROR] dts_init: \n");
...
void lcm_common_get_params(LCM_PARAMS *params)
{
	...
	if (_LCM_DTS.parsing != 0) {
		...
		dprintf(0, "[LCM][ERROR] %s: DTS is not parsed \n", __func__);
		return;
	}
}
...
void lcm_common_init(void)
{
	if (_LCM_DTS.init_size > 256) {
		dprintf(0, "[LCM][ERROR] %s: Init table overflow %d \n", __func__, _LCM_DTS.init_size);
		return;
	}
			...
			switch (init->func) {
				...
				case LCM_FUNC_CMD:
					switch (init->type) {
						...
						default:
							dprintf(0, "[LCM][ERROR] %s: %d \n", __func__, (unsigned int)init->type);
							return;
					}
					break;

				default:
					dprintf(0, "[LCM][ERROR] %s: %d \n", __func__, (unsigned int)init->func);
					return;
			}
		}
	} else {
...
void lcm_common_suspend(void)
{
	if (_LCM_DTS.suspend_size > 32) {
		dprintf(0, "[LCM][ERROR] %s: Suspend table overflow %d \n", __func__, _LCM_DTS.suspend_size);
		return;
	}
			...
			switch (suspend->func) {
				...
				case LCM_FUNC_CMD:
					switch (suspend->type) {
						...
						default:
							dprintf(0, "[LCM][ERROR] %s: %d \n", __func__, (unsigned int)suspend->type);
							return;
					}
					break;

				default:
					dprintf(0, "[LCM][ERROR] %s: %d \n", __func__, (unsigned int)suspend->func);
					return;
			}
		}
	} else {
...
void lcm_common_update(unsigned int x, unsigned int y,
...
void lcm_common_setbacklight(unsigned int level)
{
	...
	if (_LCM_DTS.backlight_size > 32) {
		dprintf(0, "[LCM][ERROR] %s: Backlight table overflow %d \n", __func__, _LCM_DTS.backlight_size);
		return;
	}
			...
			switch (backlight->func) {
				...
				case LCM_FUNC_CMD:
					switch (backlight->type) {
						case LCM_UTIL_WRITE_CMD_V1:
							lcm_util_set_write_cmd_v1(&lcm_util, &backlight->data_t5, 1);
							break;

						case LCM_UTIL_WRITE_CMD_V2:
							lcm_util_set_write_cmd_v2(&lcm_util, &backlight->data_t3, 1);
							break;

						default:
							dprintf(0, "[LCM][ERROR] %s: %d \n", __func__, (unsigned int)backlight->type);
							return;
					}
					break;

				default:
					dprintf(0, "[LCM][ERROR] %s: %d \n", __func__, (unsigned int)backlight->func);
					return;
			}
		}
	} else {
...
unsigned int lcm_common_compare_id(void)
{
	...
	if (_LCM_DTS.compare_id_size > 32) {
		dprintf(0, "[LCM][ERROR] %s: Compare ID table overflow %d \n", __func__, _LCM_DTS.compare_id_size);
		return 0;
	}
	...
	if (_LCM_DTS.parsing != 0) {
		...
		for (i = 0; i < _LCM_DTS.compare_id_size; i++) {
			...
			switch (compare_id->func) {
				...
				case LCM_FUNC_UTIL:
					lcm_util_set_data(&lcm_util, compare_id->type, &compare_id->data_t1);
					break;

				case LCM_FUNC_CMD:
					switch (compare_id->type) {
						case LCM_UTIL_WRITE_CMD_V1:
							lcm_util_set_write_cmd_v1(&lcm_util, &compare_id->data_t5, 1);
							break;

						case LCM_UTIL_WRITE_CMD_V2:
							lcm_util_set_write_cmd_v2(&lcm_util, &compare_id->data_t3, 1);
							break;

						case LCM_UTIL_READ_CMD_V2:
							lcm_util_set_read_cmd_v2(&lcm_util, &compare_id->data_t4, &compare);
							break;

						default:
							dprintf(0, "[LCM][ERROR] %s: %d \n", __func__, (unsigned int)compare_id->type);
							return 0;
					}
					break;

				default:
					dprintf(0, "[LCM][ERROR] %s: %d \n", __func__, (unsigned int)compare_id->func);
					return 0;
			}
		}
	} else {
