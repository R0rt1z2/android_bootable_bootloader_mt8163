...
extern part_t *partition;
extern int read_gpt(part_t *part);
extern unsigned long long mmc_get_region_size(unsigned int part_id);
#endif
...
#ifdef EMMC_TYPE
#ifdef MTK_NEW_COMBO_EMMC_SUPPORT
	dprintf (ALWAYS, "cmd_erase_mmc: partid %d, addr 0x%llx, size 0x%llx\n", part_id, ptn, size);
	erase_ret = emmc_erase(part_id, ptn, size);
#else
	dprintf (ALWAYS, "cmd_erase_mmc: addr 0x%llx, size 0x%llx\n", ptn, size);
