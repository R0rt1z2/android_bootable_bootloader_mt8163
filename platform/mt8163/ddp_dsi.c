...
#ifdef MACH_FPGA
#define MIPITX_I2C_BASE 0x14020000
...
//	unsigned int fmod = 30;//Fmod = 30KHz by default
	...
	for(i = DSI_MODULE_BEGIN(module);i <= DSI_MODULE_END(module);i++)
	{
		...
		MIPITX_OUTREGBIT(MIPITX_DSI_BG_CON_REG,DSI_PHY_REG[i]->MIPITX_DSI_BG_CON, RG_DSI_BG_FAST_CHARGE,1);
		...
		MIPITX_OUTREGBIT(MIPITX_DSI_BG_CON_REG,DSI_PHY_REG[i]->MIPITX_DSI_BG_CON, RG_DSI_BG_FAST_CHARGE,0);  
		...
		MIPITX_OUTREGBIT(MIPITX_DSI_PLL_TOP_REG,DSI_PHY_REG[i]->MIPITX_DSI_PLL_TOP,RG_MPPLL_PRESERVE,2);
...
void DSI_PHY_TIMCONFIG(DISP_MODULE_ENUM module, void* cmdq, LCM_DSI_PARAMS *dsi_params)
{
	...
	timcon3.CLK_HS_PRPR = 1;
	timcon3.CLK_HS_POST = 8;
	timcon3.CLK_HS_EXIT = 2;
#endif
...
UINT32 DSI_dcs_read_lcm_reg_v2(DISP_MODULE_ENUM module, void* cmdq, UINT8 cmd, UINT8 *buffer, UINT8 buffer_size)
{
	...
	for(d = DSI_MODULE_BEGIN(module);d <= DSI_MODULE_END(module);d++)
	{
		...
		do
		{
			...
			///STOP DSI
			DSI_OUTREG32(cmdq, &DSI_REG[d]->DSI_START, 0);
			DSI_Reset(module, cmdq);	/*dsi read reset*/
#endif
...

			{
				...
				DISPMSG("DSI_CMDQ_DATA0 : 0x%x \n", DSI_CMDQ_REG[d]->data[0].byte0);
				DISPMSG("DSI_CMDQ_DATA1 : 0x%x \n", DSI_CMDQ_REG[d]->data[0].byte1);
				DISPMSG("DSI_CMDQ_DATA2 : 0x%x \n", DSI_CMDQ_REG[d]->data[0].byte2);
				DISPMSG("DSI_CMDQ_DATA3 : 0x%x \n", DSI_CMDQ_REG[d]->data[0].byte3);
