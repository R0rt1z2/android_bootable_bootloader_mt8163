...
#define MMC_HOST_ID                 s_u4whoami
static u32 s_u4whoami    = -1; 
...
unsigned long long mmc_get_region_size(unsigned int part_id)
{
	return g_emmc_addr_trans[part_id].len * 512;
}
...
static int mmc_switch_part(u32 part_id)
{
    ...
    if (err)
        MSDC_DBG_PRINT(MSDC_DEBUG,("[%s]: failed to switch part_id:%d, part:%d, err=%d\n", __func__, part_id, part, err));
    return err;
}
...
u32 mmc_init_device(void)
{
    ...
    if (!blkdev_get(BOOTDEV_SDMMC)) {
		...
		MMC_HOST_ID = mmc_whoami();
		if(MMC_HOST_ID != -1){

	        ret = mmc_init(MMC_HOST_ID, MSDC_MODE_DEFAULT);
	        if (ret != 0) {
	            MSDC_ERR_PRINT(MSDC_FATAL,("[SD%u] init card failed\n",MMC_HOST_ID));
	            return ret;
	        }
    ...
    if (err == MMC_ERR_NONE) {
        ...
        if (part_config == 1) {
            ...
            *bootpart = EMMC_PART_USER;
            ...
            MSDC_ERR_PRINT(MSDC_ERROR,("[PL MTK] fail to get boot part\n"));
            return -1;
        }
    }
...
int mmc_bread_boot(blkdev_t *bdev, u32 blknr, u32 blks, u8 *buf)
{
    ...
    if(bdev && bdev->priv){
        ...
        MSDC_ERR_PRINT(MSDC_ERROR,("[%s]: invalid bdev, force the host id to 0\n", __func__, pid));
        id = 0;
    }

    if(mmc_get_boot_part(&pid)){
        MSDC_ERR_PRINT(MSDC_ERROR,("[%s]: invalid pid=%d\n", __func__, pid));
        return -1;
    }
...
int mmc_legacy_init(u32 verbose)
{
	...
	id = ((id ==0) || (id == 3)) ? mmc_whoami() : id;
	if (id != -1) {
	    bdev = &sd_dev[id];

	    //msdc_hard_reset(host);
		MSDC_TRC_PRINT(MSDC_INFO,("mmc_legacy_init(%u,%u)\n", id,MSDC_MODE_DEFAULT));
...
static int __mmc_do_erase(struct mmc_host *host,struct mmc_card *card, u64 start_addr, u64 len)
{
    ...
    if (end_addr/card->blklen > card->nblks) {
        MSDC_ERR_PRINT(MSDC_ERROR,("[MSDC%d]Erase address out of range! start<0x%llx>,len<0x%llx>,card_nblks<0x%x>\n",host->id, start_addr, len, card->nblks));
        return MMC_ERR_INVALID;
    }

    if ((err = mmc_erase_start(card, start_addr))) {
        MSDC_ERR_PRINT(MSDC_ERROR,("[MSDC%d]Set erase start addrees 0x%llx failed,Err<%d>\n",host->id,start_addr,err));
        goto out;
    }
    if ((err = mmc_erase_end(card, end_addr))){
        MSDC_ERR_PRINT(MSDC_ERROR,("[MSDC%d]Set erase end addrees 0x%llx + 0x%llx failed,Err<%d>\n",host->id,start_addr,len,err));
        goto out;
    }
    if ((err = mmc_erase(card, MMC_ERASE_TRIM))){
        MSDC_ERR_PRINT(MSDC_ERROR,("[MSDC%d]Set erase <0x%llx - 0x%llx> failed,Err<%d>\n",host->id,start_addr,start_addr + len,err));
        goto out;
    }

    MSDC_TRC_PRINT(MSDC_INFO,("[MSDC%d]0x%llx - 0x%llx Erased\n", host->id, start_addr, start_addr + len));
out:
    return err;
}
...
int mmc_do_erase(int dev_num,u64 start_addr,u64 len,u32 part_id)
{
    ...
    u32 err;
    if ((!card) || (!host) ) {
        MSDC_ERR_PRINT(MSDC_ERROR,("[mmc_do_erase] card or host is NULL\n"));
        return MMC_ERR_INVALID;
    }

    if (!len ){
        MSDC_ERR_PRINT(MSDC_ERROR,("[MSDC%d] invalid erase size! len<0x%llx>\n",host->id,len));
        return MMC_ERR_INVALID;
    }
    if ((start_addr % card->blklen) || (len % card->blklen)) {
        MSDC_ERR_PRINT(MSDC_ERROR,("[MSDC%d] non-alignment erase address! start<0x%llx>,len<0x%llx>,card_nblks<0x%x>\n",host->id,start_addr,len,card->nblks));
        return MMC_ERR_INVALID;
    }
    ...
    if ((err = mmc_switch_part(part_id))) {
        MSDC_ERR_PRINT(MSDC_ERROR,("[MSDC%d] mmc swtich failed.part<%d> error <%d> \n",host->id, part_id, err));
        return err;
    }

    if ((err = __mmc_do_erase(host, card, start_addr, len))) {
        MSDC_ERR_PRINT(MSDC_ERROR,("[MSDC%d] mmc erase failed.error <%d> \n",host->id, err));
        return err;
    }
