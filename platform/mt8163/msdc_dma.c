...
__attribute__((aligned(16))) static gpd_t msdc_gpd_pool[MSDC_MAX_NUM][MAX_GPD_POOL_SZ];
__attribute__((aligned(16))) static bd_t  msdc_bd_pool[MSDC_MAX_NUM][MAX_BD_POOL_SZ];
...

{
    ...
    if (cfg->mode == MSDC_MODE_DMA_ENHANCED) {
         ...
         */
        MSDC_ASSERT((blocks * blksz / MAX_SG_BUF_SZ) <= MAX_BD_PER_GPD);
        ...
        if (cfg->mode == MSDC_MODE_DMA_BASIC) {
             ...
             */
            MSDC_ASSERT(left_sz <= MAX_DMA_CNT);
    ...
    if (derr != MMC_ERR_NONE) {
        MSDC_ERR_PRINT(MSDC_ERROR,("[SD%d] <CMD%d> IO DMA data error (%d)\n", host->id, cmd.opcode & ~SD_CMD_BIT, derr));
...
int msdc_dma_transfer(struct mmc_host *host, struct mmc_command *cmd, struct mmc_data *data)
{
    ...
    MSDC_ASSERT(totalsz <= MAX_DMA_TRAN_SIZE);
