...
static S32 _pwrap_init_sistrobe( void )
{
  ...
  for( ind=0; ind<24; ind++)  // 24 sampling clock edge
  {
    ...
    WRAP_WR32(PMIC_WRAP_SIDLY ,0x3 - (ind & 0x3));
			...
			if( rdata == MT6325_DEFAULT_VALUE_READ_TEST ) {
				 PWRAPLOG("_pwrap_init_sistrobe [Read Test of MT6325] pass,index=%d rdata=%x\n", ind,rdata);
     			 result[0] |= (0x1 << ind);
    		}else {
				 PWRAPLOG("_pwrap_init_sistrobe [Read Test of MT6325] tuning,index=%d rdata=%x\n", ind,rdata);
			}
#endif
#ifdef SLV_6332
			...
			if( rdata == MT6323_DEFAULT_VALUE_READ_TEST ) {
				PWRAPLOG("_pwrap_init_sistrobe [Read Test of MT6323] pass,index=%d rdata=%x\n", ind,rdata);
				result[0] |= (0x1 << ind);
			}else {
				 PWRAPLOG("_pwrap_init_sistrobe [Read Test of MT6323] tuning,index=%d rdata=%x\n", ind,rdata);
			}
#endif
	 }
#ifdef SLV_6323
...
static S32 _pwrap_init_signature( U8 path )
{
	...
	if(path == 1){
		...
		//###############################
#ifdef SLV_6323
		_pwrap_wacs2_nochk(1, MT6323_DEW_WRITE_TEST, 0x5678, &rdata);
		WRAP_WR32(PMIC_WRAP_SIG_ADR,MT6323_DEW_WRITE_TEST);
#else
...
#ifdef SLV_6325
#if 0 /* Disable CRC check */
		  ret = pwrap_write_nochk(MT6325_DEW_CRC_EN, ENABLE);
		  if( ret != 0 )
		  {
			PWRAPERR("MT6325 enable CRC fail,ret=%x\n", ret);
...
S32 pwrap_init ( void )
{
 ...
 //check Wtiet test default value
  ...
  //###############################
	...
	WRAP_WR32(PMIC_WRAP_HARB_HPRIO, WACS0 | DVFSINF
					| WACS2 | WACS_P2P);
...
#endif
#ifdef SLV_6323
  pwrap_write(MT6323_DEW_WRITE_TEST, MT6323_WRITE_TEST_VALUE);
  WRAP_WR32(PMIC_WRAP_SIG_ADR, MT6323_DEW_WRITE_TEST);
  WRAP_WR32(PMIC_WRAP_SIG_VALUE, 0xAA55);
#endif
  WRAP_WR32(PMIC_WRAP_SIG_MODE, 0x1);
...
#endif
#ifdef SLV_6323
  if( (rdata&0xFFFF) != MT6323_WRITE_TEST_VALUE )
  {
    PWRAPERR("MT6323 _pwrap_status_update_test error,error code=%x, rdata=%x\n", 1, (rdata&0xFFFF));
    //return 1;
  }
#endif
#ifdef SLV_6325
...
#endif
#ifdef SLV_6323
  WRAP_WR32(PMIC_WRAP_SIG_VALUE, MT6323_WRITE_TEST_VALUE);//tha same as write test
#endif
...
#endif
#ifdef SLV_6323
  WRAP_WR32(PMIC_WRAP_SIG_ADR, MT6323_DEW_CRC_VAL);
#endif
  return 0;
}
